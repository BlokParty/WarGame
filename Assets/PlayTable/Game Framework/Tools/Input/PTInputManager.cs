...
namespace PlayTable
{
        ...
        private void RegisterLocalDelegates()
        {
                ...
                foreach (Collider collider in touch.hitsBegin.Keys)
                {
                    if (collider)
                    {
                        ...
                        if (localInput)
                        {
                            ...
                            if (!localInput.enableDrag && localInput.prefabDragToSpawn)
                            {
            ...
            PTGlobalInput.OnClicked += (PTTouch touch) =>
            {
                foreach (Collider collider in touch.hits.Keys)
                {
                    ...
                    {
                        PTLocalInput gesture = collider.GetComponent<PTLocalInput>();
                        if (gesture && gesture.OnClicked != null)
                        {
                            gesture.OnClicked(touch);
                        }
                    }
                }
            };
                ...
                foreach (PTTouchFollower draggable in touch.followers)
                {
                    if (draggable.ptLocalInput)
                    {
                        ...
                        else
                        {
                            ...
                            Vector3 targetLocalScale = draggableTransform.GetLocalScaleByWorldScale(draggableTransform.GetWorldScale());
                            draggableTransform.localScale = targetLocalScale;
                        }
                    }
    ...
    public class PTTouchFollower
    {
        ...
        public Dictionary<PTZone, float> hitZones
        {
            get
            {
                ...
                foreach (Collider hit in hits.Keys)
                {
                    ...
                    try { ret.Add(hitZone, hits[hit]); } catch { }
                }
                return ret;
            }
        }
        ...
        private const bool DEFAULT_ENABLE_DROP_ANIMATION = true;

        internal PTTouchFollower(Collider collider, PTTouch touch)
        {
            if (!collider || touch == null)
            {
                throw new InvalidCastException("[PlayTable] Parameters: Collider and PTTouch must NOT be null.");
            }
            this.touch = touch;
        ...
        public void Follow(PTTouch touch)
        {
            if (transform)
            {
                ...
                {
                    transform.position = dragWorldPositionOffset + enterPositionOffset
                            + touch.hitPoint_FixedY(transform.position, enterPosition);
                }
            }
        }
    }
    ...
    public class PTTouch
    {
        ...
        public void AddFollower(Collider collider, Vector3 dragOffset)
        {
            ...
            if (!followers.Contains(newDraggable))
            {
